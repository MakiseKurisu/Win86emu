...
static __declspec(thread) DWORD SaveSP=0;
static __declspec(thread) DWORD SaveRET=0;
...
void BX_CPU_C::cpu_loop(Bit32u max_instr_count)
{
  ...
  while (1) {
	...
	bx_pc_system.ticksTotal++;

	// check on events which occurred for previous instructions (traps)
    // and ones which are asynchronous to the CPU (hardware interrupts)
    if (BX_CPU_THIS_PTR async_event) {
		if(BX_CPU_THIS_PTR activity_state == BX_ACTIVITY_STATE_HLT)
			return;
//      if (handleAsyncEvent()) {

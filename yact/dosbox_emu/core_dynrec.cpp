...
#define CACHE_MAXSIZE	(4096)	// size of one block of generated code
#define CACHE_TOTAL		(1024*1024*8*2)	// total cache size
#define CACHE_PAGES		(512)
#define CACHE_BLOCKS	(128*1024*2)
#define CACHE_ALIGN		(4)
#define DYN_HASH_SHIFT	(4)
#define DYN_PAGE_HASH_MASK (4095)			// only 4095 is supported
#define DYN_PAGE_HASH	((DYN_PAGE_HASH_MASK+1)>>DYN_HASH_SHIFT)
#define DYN_LINKS		(16)
#define DYN_COMMANDS	(128)

#define DOSBOX_ALTERNATE_CORE CPU_Core_Simple_Run
...
CacheBlockDynRec * LinkBlocks(BlockReturn ret) {
	...
	if (temp_handler->flags & PFLAG_HASCODE) {
		...
		block=temp_handler->FindCacheBlock(temp_ip & DYN_PAGE_HASH_MASK);
...
#ifdef _ARM_
extern "C" __declspec(naked) void volatile _InterlockedIncrement8(Bit8u *ptr)	
{
/*
F3BF 8F5B            dmb         ish  
0002                 mov         r2,r0  
...
extern "C" __declspec(naked) void volatile _InterlockedDecrement8(Bit8u *ptr)	
{
/*
F3BF 8F5B            dmb         ish  
0002                 mov         r2,r0  
...
extern "C" __declspec(naked) void volatile PLI(void* Ptr)
{
//	10 F9 00 FC		PLI.W           [R0]
	__emit(0xF910);
	__emit(0xfc00);
	__emit(0x4770);
}
#else
extern "C" __declspec(naked) void volatile _InterlockedIncrement8(Bit8u *_ptr)	
{
	__asm {
		push ebp
		mov ebp,esp
...
extern "C" __declspec(naked) void volatile _InterlockedDecrement8(Bit8u *_ptr)	
{
	__asm {
		push ebp
		mov ebp,esp
...
extern "C" void volatile PLI(void* Ptr)
{
}

void __prefetch(void*)
{
}
#endif
...
Bits CPU_Core_Dynrec_Run(void) {
	for (;;) {
		...
		if (GCC_UNLIKELY(!chandler)) return DOSBOX_ALTERNATE_CORE();

		PLI(core_dynrec.runcode);

		// find correct Dynamic Block to run
		CacheBlockDynRec * block=chandler->FindCacheBlock(ip_point&DYN_PAGE_HASH_MASK);
		if (!block) {
...
//			if (!chandler->invalidation_map || (chandler->invalidation_map[ip_point&4095]<4)) {
				...
				__prefetch((void*)ip_point);
				block=CreateCacheBlock(chandler,ip_point,DYN_COMMANDS); 
/*			} else {
				// let the normal core handle this instruction to avoid zero-sized blocks
				Bitu old_cycles=CPU_Cycles;
				CPU_Cycles=1;
				Bits nc_retcode=DOSBOX_ALTERNATE_CORE();
				if (!nc_retcode) {
		...
		PLI(block->cache.start);
		
run_block:
		cache.block.running=0;
		BlockReturn ret;
//		printf("%08x",block->cache.start);
#ifdef _DEBUG
		for(int i=64; i>0; i--)
			PrevSP[i]=PrevSP[i-1];
		PrevSP[0] = reg_esp;
		for(int i=64; i>0; i--)
			PrevIP[i]=PrevIP[i-1];
		PrevIP[0] = SegPhys(cs)+reg_eip;
		for(int i=64; i>0; i--)
			PrevDyn[i]=PrevDyn[i-1];
		PrevDyn[0] = (DWORD)block->cache.start;

#endif
		Bitu old_cycles=CPU_Cycles;
		typedef BlockReturn t_func(void*p1, void* p2, void* p3, void* p4);	// TODO: change prototypes to avoid this ugly hack
		ret=((t_func*)(1+(int)core_dynrec.runcode))(block->cache.start,&Segs,&cpu_regs,&core_dynrec.readdata);
//		printf("\n");
		PIC_Ticks+=CPU_Cycles-old_cycles+1;	// at least RDTSC would increment 
...
Bits CPU_Core_Dynrec_Trap_Run(void) {
	...
	Bits ret=DOSBOX_ALTERNATE_CORE();

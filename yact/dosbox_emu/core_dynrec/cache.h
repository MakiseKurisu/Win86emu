...
// place a 32bit value into the cache word by word (as pos may be not dword aligned). As I've incorrectly made THUMB2 macros - swap halfdwords
static INLINE void cache_addt(Bit32u val) {
	cache_addw(val>>16);
	cache_addw(val&65535);
}
...
#endif
#ifdef WIN32
			if(VirtualProtect(cache_code_link_blocks,CACHE_TOTAL+CACHE_MAXSIZE+PAGESIZE_TEMP,PAGE_EXECUTE_READWRITE,&Tmp)==0)
				LOG_MSG("Setting excute permission on the code cache has failed");
#endif

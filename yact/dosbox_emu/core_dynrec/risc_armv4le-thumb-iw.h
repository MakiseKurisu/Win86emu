...
#define MOVW(dst, imm) (0xF240 + (((imm) & 0xFF) << 16) + (((imm) & 0x700) <<20) + ((dst) << 24)) + (((imm) & 0x800) >> 1)+ (((imm) & 0xF000) >> 12)

// MOVT <Rd>, #<imm16> (T1 encoding)
#define MOVT(dst, imm) (0xF2C0 + (((imm) & 0xFF) << 16) + (((imm) & 0x700) <<20) + ((dst) << 24)) + (((imm) & 0x800) >> 1)+ (((imm) & 0xF000) >> 12)
...
// ADDW< <Rd>, <Rn>, #<imm16> (T4 encoding)
#define ADDW(dst, src, imm) (0xF200 + (((imm) & 0xFF) << 16) + (((imm) & 0x700) <<20) + ((dst) << 24) + (((imm) & 0x800) >> 1)+  (src))

// SUBW <Rd>, <Rn>, #<imm16> (T4 encoding)
#define SUBW(dst, src, imm) (0xF2A0 + (((imm) & 0xFF) << 16) + (((imm) & 0x700) <<20) + ((dst) << 24) + (((imm) & 0x800) >> 1)+  (src))

#define CBZ(reg, imm) (0xB100 + (reg) + ((imm) << 3))
#define CBNZ(reg, imm) (0xB900 + (reg) + ((imm) << 3))
...
//LDRW  Rt, [Rn, #imm12] (Encoding T3)
#define LDRW(dst_reg, off_reg, imm) (0xF8D0 + ((imm) << 16) + (off_reg) + ((dst_reg) << 28))
//LDRBH Rt, [Rn, #imm12]
#define LDRHW(dst_reg, off_reg, imm) (0xF8B0 + ((imm) << 16) + (off_reg) + ((dst_reg) << 28))
//LDRBW Rt, [Rn, #imm12]
#define LDRBW(dst_reg, off_reg, imm) (0xF890 + ((imm) << 16) + (off_reg) + ((dst_reg) << 28))
...
//STRW Rt, [Rn, #imm12] (Encoding T3)
#define STRW(src_reg, off_reg, imm) (0xF8C0 + ((imm) << 16) + (off_reg) + ((src_reg) << 28))
//STRHW Rt, [Rn, #imm12] (Encoding T2)
#define STRHW(src_reg, off_reg, imm) (0xF8A0 + ((imm) << 16) + (off_reg) + ((src_reg) << 28))
//STRBW Rt, [Rn, #imm12] (Encoding T2)
#define STRBW(src_reg, off_reg, imm) (0xF880 + ((imm) << 16) + (off_reg) + ((src_reg) << 28))
...
static bool gen_mov_memreg_to_reg(HostReg dest_reg, HostReg addr_reg, Bitu size) {
// TODO: unaligned!!
	if(addr_reg>7)
	{
		cache_checkinstr(2);
		gen_mov_regs(templo1,addr_reg);
		addr_reg=templo1;
	}
	...
	switch (size) {
		case 4:
			cache_addw( LDR_IMM(dest_reg, addr_reg, 0) );      // ldr dest_reg, [addr_reg]
			return true;
		case 2:
			cache_addw( LDRH_IMM(dest_reg, addr_reg, 0) );      // ldrh dest_reg, [addr_reg]
			return true;
		case 1:
			cache_addw( LDRB_IMM(dest_reg, addr_reg, 0) );      // ldr dest_reg, [addr_reg]
			return true;
	}
	return false;
}

static bool gen_mov_reg_to_memreg(HostReg addr_reg, HostReg val_reg, Bitu size) {
// TODO: unaligned!!
	if(addr_reg>7)
	{
		cache_checkinstr(2);
	...
	switch (size) {
		case 4:
			cache_addw( STR_IMM(val_reg, addr_reg, 0) );      // ldr dest_reg, [addr_reg]
			return true;
		case 2:
			cache_addw( STRH_IMM(val_reg, addr_reg, 0) );      // ldrh dest_reg, [addr_reg]
			return true;
		case 1:
			cache_addw( STRB_IMM(val_reg, addr_reg, 0) );      // ldr dest_reg, [addr_reg]
			return true;
	}
	return false;
}
...
static void gen_mov_dword_to_reg_imm(HostReg dest_reg,Bit32u imm) {
	...
	else if (imm < 65535) // we can use a single MOVW instruction
	{
		cache_checkinstr(4);
		cache_addd(MOVW(dest_reg, imm));
	}
	else //use a MOVW followed by a MOVT
	{
		cache_checkinstr(8);
		cache_addd(MOVW(dest_reg, imm & 0xffff));
		cache_addd(MOVT(dest_reg, (imm & 0xffff0000) >> 16));
	}
	...
	switch (size) {
...
#endif
			{
				if ((data >= addr_data) && (data < addr_data + 4096) && (((data - addr_data) & 3) == 0)) {
					cache_checkinstr(4);
					cache_addd( LDRW(dest_reg, addr_reg, data-addr_data));
...
#endif
			{
				if ((data >= addr_data) && (data < addr_data + 4096) && (((data - addr_data) & 1) == 0)) {
					cache_checkinstr(4);
					cache_addd( LDRHW(dest_reg, addr_reg, data-addr_data));
					//cache_addw( MOV_LO_HI(templo2, addr_reg) );      // mov templo2, addr_reg
		...
		case 1:
			if ((data >= addr_data) && (data < addr_data + 4096)) {
				cache_checkinstr(4);
				cache_addd( LDRBW(dest_reg, addr_reg, data-addr_data));
				//cache_addw( MOV_LO_HI(templo2, addr_reg) );      // mov templo2, addr_reg
...
static void INLINE gen_mov_word_to_reg_imm(HostReg dest_reg,Bit16u imm) {
	//LOG_MSG("Generating MOV imm: reg 0x%x, imm 0x%x (ins=0x%08x)", dest_reg, imm, MOVW(dest_reg, imm));
	if (imm < 256)
	{
		//we can use a 16-bit instruction
		cache_checkinstr(2);
		cache_addw(MOV_IMM(dest_reg, imm) );      // mov dest_reg, #(imm)
	}
	else
	...
	switch (size) {
		case 4:
...
#endif
			{
				if ((data >= addr_data) && (data < addr_data + 4096) && (((data - addr_data) & 3) == 0)) {
					cache_checkinstr(4);
					cache_addd(STRW(src_reg, addr_reg, data - addr_data));
					//cache_addw( MOV_LO_HI(templo2, addr_reg) );      // mov templo2, addr_reg
		...
		case 2:
...
#endif
			{
				if ((data >= addr_data) && (data < addr_data + 4096) && (((data - addr_data) & 1) == 0)) {
					cache_checkinstr(4);
					cache_addd(STRHW(src_reg, addr_reg, data-addr_data));
					//cache_addw( MOV_LO_HI(templo2, addr_reg) );      // mov templo2, addr_reg
		...
		case 1:
			if ((data >= addr_data) && (data < addr_data + 4096)) {
				cache_checkinstr(4);
				cache_addd(STRBW(src_reg, addr_reg, data-addr_data));
				//cache_addw( MOV_LO_HI(templo2, addr_reg) );      // mov templo2, addr_reg
...
static void gen_add_imm(HostReg reg,Bit32u imm) {
	...
	{
		// we can use ADDW here
		cache_checkinstr(4);
		cache_addd(ADDW(reg, reg, imm));
	}
	else if (imm2 <= 4095)
	{
		cache_checkinstr(4);
		cache_addd(SUBW(reg, reg, imm2));
	}
	else {
...
static void gen_sub_direct_word(void* dest,Bit32u imm,bool dword) {
	...
	{
		// we can use SUBW here
		cache_checkinstr(4);
		cache_addd(SUBW(templo3, templo3, imm));
	}
	else if (imm2 <= 4095)
	{
		cache_checkinstr(4);
		cache_addd(ADDW(templo3, templo3, imm2));
	} else {if (val_single_shift(imm2, &scale)) {
			cache_checkinstr((scale)?6:4);
			cache_addw( MOV_IMM(templo1, imm2 >> scale) );      // mov templo1, #(~imm >> scale)
			if (scale) {
				cache_addw( LSL_IMM(templo1, templo1, scale) );      // lsl templo1, templo1, #scale
...
static void gen_call_function_helper(void * func) {
	gen_mov_dword_to_reg_imm(templo1,(Bit32u)func);	// 4+4 bytes
	cache_addw( BLX(templo1) );      // blx templo1
/*
	if (((Bit32u)cache.pos & 0x03) == 0) {
...
static void INLINE gen_call_function_raw(void * func) {
	if(func==&dynrec_push_dword)
	{
		gen_sub_direct_word(&reg_esp,4,true);
		// HACK: this is unnecessary as gen_sub_direct_word already placed new ESP value into templo3: 
		gen_mov_memval_to_reg(templo3, &reg_esp, 4);
		gen_mov_reg_to_memreg(templo3,FC_OP1,4);
	}else if(func==&dynrec_pop_dword)
	{
		gen_mov_memval_to_reg(templo3, &reg_esp, 4);
		gen_mov_memreg_to_reg(FC_OP1,templo3,4);
		gen_add_direct_word(&reg_esp,4,true);
	}else
	{
		cache_checkinstr(12);
...
static void gen_run_code(void) {
	Bit8u *pos1, *pos2, *pos3;
// generating THUMB code!
// input: R0 == code_ptr
// R1 == &Segs
// R2 == &cpu_regs
// R3 == &core_dynrec.readdata

	cache_addd(0x4FF0E92D);			//  PUSH.W          {R4-R11,LR}

	cache_addw( ADD_IMM3(HOST_r0, HOST_r0, 1) );      // add r0, r0, #1
	cache_addw( MOV_HI_LO(FC_SEGS_ADDR,HOST_r1) );
	cache_addw( MOV_HI_LO(FC_REGS_ADDR,HOST_r2) );
	cache_addw( MOV_HI_LO(readdata_addr,HOST_r3) );
	cache_addw( ADD_LO_PC_IMM(HOST_r3, 6) );      // add lr, pc, #6
	cache_addw( ADD_IMM3(HOST_r3, HOST_r3, 1) );      // add r3, r3, #1
	cache_addw(0xb408);      // push {r3}
	cache_addw( BX(HOST_r0) );			// bx r0
	cache_addd(0x8FF0E8BD);			//  POP.W          {R4-R11,PC}
	...
	if (((Bit32u)pos & 0x03) == 0)
	{
		...
		switch (flags_type) {
			...
			case t_ADDd:
				...
				*(Bit16u*)(pos+2)=B_FWD(4);							// b after_call (pc+6)
				break;
			case t_ORb:
			case t_ORw:
			case t_ORd:
			...
			case t_TESTw:
			case t_TESTd:
				*(Bit16u*)pos=B_FWD(6);								// b after_call (pc+8)
				break;
			case t_INCb:
			case t_INCw:
			case t_INCd:
...
static void gen_mov_seg16_to_reg(HostReg dest_reg,Bitu index) {
	...
	cache_addd(LDRHW(dest_reg, FC_SEGS_ADDR, index));
}
...
static void gen_mov_seg32_to_reg(HostReg dest_reg,Bitu index) {
	...
	cache_addd(LDRW(dest_reg, FC_SEGS_ADDR, index));
}
...
static void gen_add_seg32_to_reg(HostReg reg,Bitu index) {
	...
	cache_addd(LDRW(templo1, FC_SEGS_ADDR, index));
	cache_addw(ADD_REG(reg, reg, templo1));
}
...
static void gen_mov_regval16_to_reg(HostReg dest_reg,Bitu index) {
	...
	cache_addd( LDRHW(dest_reg, FC_REGS_ADDR, index));
	/*
...
static void gen_mov_regval32_to_reg(HostReg dest_reg,Bitu index) {
	...
	cache_addd( LDRW(dest_reg, FC_REGS_ADDR, index));
	/*
	cache_addw( MOV_LO_HI(templo2, FC_REGS_ADDR) );      // mov templo2, FC_REGS_ADDR
...
//	cache_addw( LDRB_IMM(dest_reg, templo2, index) );      // ldrb dest_reg, [templo2, #index]
	cache_addd(LDRBW(dest_reg, FC_REGS_ADDR, index));
}
...
static void gen_add_regval32_to_reg(HostReg reg,Bitu index) {
	...
	cache_addd(LDRW(templo2, FC_REGS_ADDR, index));
	cache_addw(ADD_REG(reg, reg, templo2));
}
...
static void gen_mov_regval16_from_reg(HostReg src_reg,Bitu index) {
	...
	cache_addd(STRHW(src_reg, FC_REGS_ADDR, index));
}
...
static void gen_mov_regval32_from_reg(HostReg src_reg,Bitu index) {
	...
	cache_addd(STRW(src_reg, FC_REGS_ADDR, index)); 
	/*
	cache_addw( MOV_LO_HI(templo1, FC_REGS_ADDR) );      // mov templo1, FC_REGS_ADDR
...
static void gen_mov_regbyte_from_reg_low(HostReg src_reg,Bitu index) {
	...
	cache_addd(STRBW(src_reg, FC_REGS_ADDR, index));
	//cache_addw( MOV_LO_HI(templo1, FC_REGS_ADDR) );      // mov templo1, FC_REGS_ADDR

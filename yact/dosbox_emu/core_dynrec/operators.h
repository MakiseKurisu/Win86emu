...
static bool DRC_CALL_CONV dynrec_div_byte(Bit8u op) {
	Bitu val=op;
//TODO	if (val==0) return CPU_PrepareException(0,0);
        if (val==0) val=1;
	Bitu quo=reg_ax / val;
	Bit8u rem=(Bit8u)(reg_ax % val);
	Bit8u quo8=(Bit8u)(quo&0xff);
//TODO	if (quo>0xff) return CPU_PrepareException(0,0);
	reg_ah=rem;
	reg_al=quo8;
	return false;
}
...
//TODO	if (quo!=(Bit16s)quo8s) return CPU_PrepareException(0,0);
	reg_ah=rem;
	reg_al=quo8s;
	return false;
}
...
//TODO	if (quo!=(Bit32u)quo16) return CPU_PrepareException(0,0);
	reg_dx=rem;
	reg_ax=quo16;
	return false;
}
...
//TODO	if (quo!=(Bit32s)quo16s) return CPU_PrepareException(0,0);
	reg_dx=rem;
	reg_ax=quo16s;
	return false;
}
...
//TODO	if (quo!=(Bit64u)quo32) return CPU_PrepareException(0,0);
	reg_edx=rem;
	reg_eax=quo32;
	return false;
}
...
//TODO	if (quo!=(Bit64s)quo32s) return CPU_PrepareException(0,0);
	reg_edx=rem;
	reg_eax=quo32s;
	return false;
}
...
#ifdef _ARM_
	Bit32u new_esp=reg_esp-2;
	mem_writew(new_esp ,value);
#else
...
#ifdef _ARM_
	Bit32u new_esp=reg_esp-4;
	mem_writed(new_esp ,value);
#else

...
static bool MakeCodePage(Bitu lin_addr,CodePageHandlerDynRec * &cph) {
	Bit8u rdval;
#ifdef _DEBUG
	if(lin_addr<0x10000 || lin_addr>0x80000000)
		return true;
#endif
...
#define MOV_SEG_PHYS_TO_HOST_REG(host_reg, seg_index) if((seg_index)==DRC_SEG_FS) gen_mov_seg32_to_reg(host_reg,(DRC_PTR_SIZE_IM)(DRCD_SEG_PHYS(seg_index)) - (DRC_PTR_SIZE_IM)(&Segs)); else gen_mov_dword_to_reg_imm(host_reg,0)
#define ADD_SEG_PHYS_TO_HOST_REG(host_reg, seg_index) if((seg_index)==DRC_SEG_FS) gen_add_seg32_to_reg(host_reg,(DRC_PTR_SIZE_IM)(DRCD_SEG_PHYS(seg_index)) - (DRC_PTR_SIZE_IM)(&Segs))
...
// no need in this project:	gen_sub_direct_word(&CPU_Cycles,decode.cycles,true);
}
...
//	if (tlb_addr) {
		*((Bit8u*)(&core_dynrec.readdata))=host_readb((HostPt)address);
		return false;
//	} else {
...
//	if (tlb_addr) {
		host_writeb((HostPt)address,val);
		return false;
...
//		HostPt tlb_addr=get_tlb_read(address);
//		if (tlb_addr) {
			*((Bit16u*)(&core_dynrec.readdata))=host_readw((HostPt)address);
			return false;
...
//		HostPt tlb_addr=get_tlb_read(address);
//		if (tlb_addr) {
			*((Bit32u*)(&core_dynrec.readdata))=host_readd((HostPt)address);
			return false;
...
//		HostPt tlb_addr=get_tlb_write(address);
//		if (tlb_addr) {
			host_writew((HostPt)address,val);
			return false;
...
//		HostPt tlb_addr=get_tlb_write(address);
//		if (tlb_addr) {
			host_writed((HostPt)address,val);
			return false;
...
#else
	gen_mov_memreg_to_reg(reg_dst,reg_addr,1);
#endif
}
...
static void dyn_write_byte(HostReg reg_addr,HostReg reg_val) {
	...
	dyn_check_exception(FC_RETOP);
#else
	gen_mov_reg_to_memreg(reg_addr, reg_val, 1);
#endif
}
...
#else
	gen_mov_memreg_to_reg(reg_dst,reg_addr,dword?4:2);
#endif
}
...
//	if (!dword) gen_extend_word(false,reg_val);
	...
	dyn_check_exception(FC_RETOP);
#else
	gen_mov_reg_to_memreg(reg_addr, reg_val, dword?4:2);
#endif
}

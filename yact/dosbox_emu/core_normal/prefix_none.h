 ...
 */
	...
	CASE_B(0xf0)												/* LOCK */
		...
		goto restart_opcode;	// we need to execute the next command too
		break;
	CASE_B(0xf1)												/* ICEBP */
	...
	CASE_B(0xfe)												/* GRP4 Eb */
		{
			...
			case 0x00:										/* INC Eb */
				...
				{
					GetEAa;
					Bit8u* addr=(Bit8u*)(eaa);
					LoadCF;
					lf_resb=_InterlockedIncrement8(addr);//++(*addr);  -- normal core is used only after LOCK prefix, so use interlocked inc/dec
					lf_var1b=lf_resb-1;	
					lflags.type=t_INCb;									
				}
				break;		
			case 0x01:										/* DEC Eb */
				...
				} else
				{
					...
					Bit8u* addr=(Bit8u*)(eaa);
					LoadCF;
					lf_resb=_InterlockedDecrement8(addr);	//--(*addr);
					lf_var1b=lf_resb+1;	
					lflags.type=t_DECb;									
				}
				break;
	...
	CASE_W(0xff)												/* GRP5 Ew */
		{
			...
			case 0x00:										/* INC Eb */
				...
				{
					GetEAa;
					Bit16u* addr=(Bit16u*)(eaa);
					LoadCF;
					lf_resw=_InterlockedIncrement16((volatile short*)addr);//++(*addr);  -- normal core is used only after LOCK prefix, so use interlocked inc/dec
					lf_var1w=lf_resw-1;	
					lflags.type=t_INCw;									
				}
				break;		
			case 0x01:										/* DEC Eb */
				...
				} else
				{
					...
					Bit16u* addr=(Bit16u*)(eaa);
					LoadCF;
					lf_resw=_InterlockedDecrement16((volatile short*)addr);	//--(*addr);
					lf_var1w=lf_resw+1;	
					lflags.type=t_DECw;									
				}
				break;

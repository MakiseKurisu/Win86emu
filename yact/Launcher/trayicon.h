...
public:
    ...
    CTrayIcon(LPCTSTR szTip, HICON hIcon)
    {
        Create(szTip, hIcon);
    }
    virtual ~CTrayIcon()
...
protected:
    ...
    // Operations
    BOOL Create(LPCTSTR szTip, HICON hIcon, UINT Menu = 0)
    { //Menu may be used by derived classes
        ...
        if (OK)
        {
            ...
            NID.uCallbackMessage = RegisterWindowMessageA("CTrayIcon");
            _tcscpy(NID.szTip, szTip);
...
public:
    ...
    CMenuTrayIcon(LPCTSTR szTip, HICON hIcon, UINT Menu = 0, UINT Default = 0, BOOL ByPos = FALSE) : DoubleClicked(false)
    {
        Create(szTip, hIcon, Menu, Default, ByPos);
    }
    ...
    virtual LRESULT WindowProc(UINT Message, WPARAM wParam, LPARAM lParam)
    {
            ...
            && wParam == NID.uID)
        {
            ...
            switch (LOWORD(lParam))
            {
                ...
                case WM_LBUTTONUP: {
                    if (DoubleClicked)
                    { // Make sure that if the default deletes the icon, that the next icon doesn't get our WM_LBUTTONUP notification (some icons use it).
                        DoubleClicked = false;
...
public:
    ...
    void SetIcons(const UINT IconIDs[])
    { // NULL Terminated Array of Icon Recource IDs to animate
...
public:
    ...
    CAniTrayIcon(LPCTSTR szTip, HICON hIcon)
    {
        Create(szTip, hIcon);
    }
    virtual ~CAniTrayIcon()
    {
        StopAnimating();
    }
    ...
    HICON GetIcon() const
    {
        return CTrayIcon::GetIcon();
    }
    BOOL  SetIcon(HICON hIcon)
    {
        return CTrayIcon::SetIcon(hIcon);
    }

    UINT SetTimer(UINT nIDEvent, UINT nElapse, void (CALLBACK* lpfnTimer)(HWND, UINT, UINT, DWORD))
    {
        return CTrayIcon::SetTimer(nIDEvent, nElapse, lpfnTimer);
    }
    BOOL KillTimer(int nIDEvent)
    {
        return CTrayIcon::KillTimer(nIDEvent);
    }
...
public:
    ...
    HICON GetIcon() const
    {
        return CMenuTrayIcon::GetIcon();
    }
    BOOL  SetIcon(HICON hIcon)
    {
        return CMenuTrayIcon::SetIcon(hIcon);
    }

    UINT SetTimer(UINT nIDEvent, UINT nElapse, void (CALLBACK* lpfnTimer)(HWND, UINT, UINT, DWORD))
    {
        return CMenuTrayIcon::SetTimer(nIDEvent, nElapse, lpfnTimer);
    }
    BOOL KillTimer(int nIDEvent)
    {
        return CMenuTrayIcon::KillTimer(nIDEvent);
    }
